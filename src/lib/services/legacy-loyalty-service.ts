...
export type LegacyLoyaltyResolveResult =
  | { status: 'resolved'; customerId: string; source: 'legacy_existing_shadow' | 'legacy_shadow' }
  | { status: 'not_enabled' | 'not_found' | 'shadow_disabled' }
  | { status: 'duplicate'; count: number }

const PROVIDER = 'passkit_lovable'

export async function resolveLegacyLoyaltyScan(
  studioId: string,
  scannedValue: string
): Promise<LegacyLoyaltyResolveResult> {
  const legacy = await getLegacySettings(studioId)
  if (!legacy.enabled || legacy.resolve_on_scan === false || !legacy.legacy_studio_id) {
    return { status: 'not_enabled' }
  }

  const legacySupabase = createLegacyClient()
  ...
  if (rows.length > 1) {
    await logAudit(studioId, 'legacy_loyalty.duplicate_lookup', null, {
      scannedValue,
      count: rows.length,
    })
    return { status: 'duplicate', count: rows.length }
  }
  ...
  if (existingLink?.customer_id) {
    return { status: 'resolved', customerId: existingLink.customer_id, source: 'legacy_existing_shadow' }
  }

  if (legacy.create_shadow_on_resolve === false) return { status: 'shadow_disabled' }
  ...
  return { status: 'resolved', customerId, source: 'legacy_shadow' }
}

export async function resolveLegacyLoyaltySearch(
  studioId: string,
  searchValue: string
): Promise<LegacyLoyaltyResolveResult> {
  const legacy = await getLegacySettings(studioId)
  ...
  const rows = await searchLegacyMembers(legacy.legacy_studio_id, searchValue)
  if (rows.length === 0) return { status: 'not_found' }
  if (rows.length > 1) {
    await logAudit(studioId, 'legacy_loyalty.ambiguous_search', null, {
      searchValue,
      count: rows.length,
    })
    return { status: 'duplicate', count: rows.length }
  ...
  const customerId = await findOrCreateShadowCustomer(studioId, legacy.legacy_studio_id, legacyMember)
  await upsertLegacyLink(studioId, customerId, legacy.legacy_studio_id, legacyMember, searchValue.trim())
  await logAudit(studioId, 'legacy_loyalty.search_resolved', customerId, {
    legacy_studio_id: legacy.legacy_studio_id,
    legacy_member_id: legacyMember.member_id,
    legacy_passkit_id: legacyMember.passkit_id,
...
async function getLegacySettings(studioId: string): Promise<LegacyLoyaltySettings> {
  ...
  return (settings.legacy_loyalty ?? {}) as LegacyLoyaltySettings
}
...
function createLegacyClient() {
  ...
  if (!legacyUrl || !legacyServiceRoleKey) {
    throw new Error('Legacy loyalty is not configured')
  }
  return createSupabaseClient(legacyUrl, legacyServiceRoleKey)
}

async function searchLegacyMembers(legacyStudioId: string, searchValue: string) {
  const legacySupabase = createLegacyClient()
  const raw = searchValue.trim()
  const compact = raw.replace(/\s+/g, '')
  const escaped = escapeIlike(raw)
  const compactEscaped = escapeIlike(compact)
  const byIdValues = Array.from(new Set([raw, compact].filter(Boolean)))
  const results = new Map<string, LegacyMember>()

  const exactQueries = byIdValues.flatMap((value) => [
    legacySupabase
      .from('legacy_loyalty_members_export')
      .select('*')
      .eq('studio_id', legacyStudioId)
      .eq('member_id', value)
      .limit(5),
    legacySupabase
      .from('legacy_loyalty_members_export')
  ...
  if (compact && compact !== raw) {
    fuzzyQueries.push(
      legacySupabase
        ...
        .select('*')
        .eq('studio_id', legacyStudioId)
        .ilike('phone', `%${compactEscaped}%`)
        .limit(5)
    )
  }

  const responses = await Promise.all([...exactQueries, ...fuzzyQueries])
  for (const response of responses) {
    if (response.error) throw new Error(response.error.message)
    for (const row of (response.data ?? []) as LegacyMember[]) {
